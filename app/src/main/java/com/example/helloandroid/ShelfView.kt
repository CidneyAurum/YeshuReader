...
class ShelfView(private val act: Activity) : FrameLayout(act) {
    ...
    private fun batchDelete() {
        ...
        AlertDialog.Builder(act)
            .setTitle("批量删除")
            .setMessage("删除选中的 ${selected.size} 本书？文件将一并删除。")
            .setPositiveButton("删除") { _, _ ->
                selected.forEach { id ->
                    db.getBook(id)?.let { b ->
                        File(act.filesDir, b.fileName).delete()
                        CoverStore.file(act, id).delete()
                        db.deleteBook(id)
                    }
                }
                selected.clear()
    ...
    fun refresh(query: String? = null) {
        ...
        if (!searching && filterRow != null && filterHost != null) {
            ...
            listOf("全部" to "all", "在读" to "reading", "未读" to "unread", "读完" to "done").forEach { (label, key) ->
            ...
            android.util.Log.d("SHELF", "chips shown count=${filterRow!!.childCount} searching=$searching")
        } else {
            filterHost?.visibility = View.GONE
            android.util.Log.d("SHELF", "chips hidden searching=$searching")
        }
    ...
    private fun bookLongPress(b: Book): Boolean {
        val options = arrayOf("打开", "详情", "AI 摘要", "笔记", "移动到…", "导出笔记", "删除")
        AlertDialog.Builder(act)
            .setTitle(b.title)
            .setItems(options) { _, which ->
                when (which) {
                    ...
                    1 -> bookDetail(b)
                    2 -> aiSummary(b)
                    3 -> (act as MainActivity).showNotes(b.id)
                    4 -> moveDialog(targetBook = b.id)
                    5 -> exportNotes(b)
                    6 -> AlertDialog.Builder(act)
                        .setTitle("删除")
                        .setMessage("删除《" + b.title + "》？")
                        .setPositiveButton("删除") { _, _ ->
                            File(act.filesDir, b.fileName).delete()
                            db.deleteBook(b.id)
                            refresh()
