...
class SettingsView(private val act: Activity) : FrameLayout(act) {
    ...
    init {
        ...
        etKey = box.addView_ret(buildEdit(d, "sk-...", password = true)) as EditText
        ...
        val test = TextView(act).apply {
            ...
            setOnClickListener { v ->
                ...
                Thread {
                    var msg: String
                    try {
                        val cfg = AiClient.Config(AiClient.normalizeBase(url), key.ifEmpty { "sk-test" }, model)
