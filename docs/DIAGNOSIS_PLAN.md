...
# AI 功能第二轮重构：逻辑结构与实现效果

上一轮把「摘要/问答/出题」统一到正规路径后，本轮继续深挖**逻辑结构**与**实际输出效果**，
全部结论都来自真机 + 真实服务商的实测，不是静态推断。

## 一、聊天链路（逻辑结构）

1. **章节上下文只取前 40 块**：`currentChapterText()` 用 `blocks.drop(start).take(40)`，
   长章节的后半段完全没有上下文。→ 改为传整章，由锚点抽样在预算内挑选。
2. **章节上下文是「拼接文本」**：`joinToString("
")` 再按换行切回块，段落内部含换行时
   一个块会被切成多个，**段落编号因此比阅读器多**。→ 新增
   `buildAnchoredContextFromBlocks()`，直接传块列表，编号逐一对齐。
3. **编号口径不一致**：聊天里的 `[PARAGRAPH:n]` 是全书口径，而片段内从 1 重新计数。
   → `buildAnchoredContextFromBlocks/FromText` 支持 `paragraphOffset`/`chapterOffset`，
   阅读器传入当前章的编号起点。
4. **回答里的引用不可点**：`loadHistory()` 回放历史消息时直接 `bubble(...)`，没有链接化，
   重进页面后旧回答的引用点不动。→ 链接化放进 `bubble()`，实时与回放一致。
5. **没有费用反馈**：聊天是一问一计费，用户无从判断问一句花多少。→ 每轮追加
   「本轮用时 3.9s · 输入 247 / 输出 114 tokens」脚注（只上屏，不入库、不回放）。
6. **历史预算静默丢弃**：超过 6000 字符的更早轮次被悄悄丢掉，模型像失忆。→ 明确提示。
7. **语言检测重复实现**：`ChatView.languageHint` 是 `DocumentAiService.detectLanguage` 的
   影子实现（注释还写着「尚未落地」，其实早已落地）。→ 统一复用。

## 二、解析器（实现效果）

8. **`<head>` 元数据混进正文**：`extractXhtml` 把 `<title>` 也当标题块，于是
   **目录里每章出现两次**（`<title>` 一次、`<h1>` 一次），章节计数也虚高。
   → 进入 `<head>` 即跳过。
9. **EPUB3 导航页被当成正文**：`nav.xhtml` 常出现在 spine 里，它的 `<h2>` 和 `<li>`
   被当作内容——目录最前面多出一条书名，**且后续所有段落编号整体后移**
   （AI 引用因此指到错误位置）。→ 按 `properties="nav"` 排除导航文档。
...
| | 修复前 | 修复后 |
|---|---|---|
| 模型看到的段落编号 | `[PARAGRAPH:7]` `[PARAGRAPH:8]` | `[PARAGRAPH:1]` `[PARAGRAPH:2]` |
| 与文档真实结构（2 章各 1 段） | 不一致，点引用报「超出文档范围」 | **完全一致** |
| 目录条目 | 每章重复两遍 | 每章一条 |
| 点引用 | 无法定位 | 跳回阅读器对应位置 |

## 四、本轮顺带修掉的自身缺陷

10. **打开聊天必崩**（我上一轮引入）：`onDetachedFromWindow` 先 `ioExecutor.shutdownNow()`，
    紧接着 `pauseReadSession()` 又要通过它落库 → `RejectedExecutionException`。
    从阅读器进「和书聊聊」直接闪退。→ 不再关闭执行器（单线程 daemon，闲置不占资源），
    并加 `submitToIo` 兜底，避免将来任何关闭时序把界面打崩。
    *教训：上一轮 D13 只测了滚动，没测「导航离开阅读器」，所以漏掉了。*
11. **死代码**：`castAction`（人物速查）早已并入「前情提要」，函数却留在代码里。
12. **实况测试瞬态清单**：补 `SSLException` 与 5xx（实测遇到 503 过载）。

测试 150 → 154 全绿；新增 4 条防回归测试分别锁住段落编号、head 元数据、导航页、
以及片段编号偏移。
